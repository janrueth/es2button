...
#include "include/Include/Controller/EpsonScan2.h"
#include "include/Include/ES2Command/ES2CommandAPI.h"
...
class ESListener : public IESScannerDelegate {
 ...
 public:
  ...
  ESErrorCode init(USBDevice* dev, const char* prog) {
    _dev = *dev;
    ESErrorCode eErrorCode = ESCreateScanner(kESCommandTypeESCI2, &_scanner);
...
int main(int argc, char* argv[]) {
  ...
  if (optind >= argc) {
    printf("Expected path to program after options\n\n");
    print_usage(argv);
    return 1;
  }
  ...
  err = listener.init(&dev, argv[optind]);
