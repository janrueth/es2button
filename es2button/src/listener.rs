...
pub(crate) fn run_listener(device: UsbDevice, program: PathBuf) {
    let scanner = es2command::Scanner::default();

    let connect_info = serde_json::to_string(&UsbConnectInfo::from(device.clone())).unwrap();
    trace!("Connecting: {}", &connect_info);
    scanner.set_connection(&connect_info).unwrap();
    ...
    let mut signals = signal_hook::iterator::Signals::new([SIGINT, SIGPIPE]).unwrap();
